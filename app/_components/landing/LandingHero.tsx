...
function TopBar({ onDark }: { onDark: boolean }) {
  ...
  return (
    <div className="flex w-full flex-wrap items-center justify-between gap-4">
      ...
      <nav className="flex flex-wrap items-center justify-end gap-x-4 gap-y-1 text-xs font-semibold">
...
function HeroClinic({ config }: { config: LandingConfig }) {
  return (
    <header className="relative w-full overflow-hidden bg-[#e9e4dc]">
      ...
      <div className="relative border-b border-stone-800/10 bg-white/30 px-6 py-5 backdrop-blur-sm md:px-10">
        <div className="mx-auto max-w-5xl">
          <TopBar onDark={false} />
        </div>
      </div>

      <div className="relative mx-auto max-w-4xl px-6 pb-24 pt-16 text-center md:px-8 md:pb-32 md:pt-20">
        ...
        <ul className="mx-auto mt-12 max-w-2xl space-y-5 text-left sm:text-center">
...
function HeroWellness({ config }: { config: LandingConfig }) {
  return (
    <header className="relative w-full overflow-hidden bg-[#f0f0f2]">
      <div className="border-b border-gray-900/10 bg-white/70 px-6 py-5 backdrop-blur-sm md:px-10">
        <div className="mx-auto max-w-6xl">
          <TopBar onDark={false} />
        </div>
      </div>

      <div className="relative mx-auto max-w-6xl px-6 pb-20 pt-12 md:px-10 md:pb-28 md:pt-16">
        <div className="grid gap-10 lg:grid-cols-12 lg:gap-8 lg:items-start">
          <div className="lg:col-span-7">
            ...
            <p className="mt-6 max-w-xl text-base leading-relaxed text-gray-600 md:text-lg">{config.subhead}</p>
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:col-span-5 lg:grid-cols-1 lg:gap-4">
            {config.bullets.map((line, i) => (
              ...
              >
                <p className={`text-[10px] font-bold uppercase tracking-wider ${i === 2 ? "text-white/60" : "text-[#0d6e5c]"}`}>
                  {String(i + 1).padStart(2, "0")}
                </p>
                <p className={`mt-2 text-sm font-medium leading-snug ${i === 2 ? "text-white/95" : "text-gray-800"}`}>
                  {line}
                </p>
              </div>
            ))}
